...
export class MenuCategoryListComponent extends ComponentBase implements OnInit {
  ...
  readonly permissions = {
    create: PERMISSIONS.RESTAURANT.MENU.CATEGORIES.CREATE, // Quyền tạo mới danh mục
    edit: PERMISSIONS.RESTAURANT.MENU.CATEGORIES.EDIT, // Quyền chỉnh sửa danh mục
    delete: PERMISSIONS.RESTAURANT.MENU.CATEGORIES.DELETE, // Quyền xóa danh mục
  };
  ...
  // Dữ liệu hiển thị
  categories = signal<MenuCategoryDto[]>([]); // Danh sách danh mục món ăn (client-side filtering)
  selectedCategories: MenuCategoryDto[] = []; // Các danh mục được chọn để xóa bulk
  loading = false; // Trạng thái loading khi gọi API

  // Hằng số
  private readonly ENTITY_NAME = 'danh mục món ăn'; // Tên entity dùng trong thông báo
  ...
  deleteCategory(category: MenuCategoryDto) {
  ...
  private performDeleteCategory(category: MenuCategoryDto) {
    this.menuCategoryService.delete(category.id).subscribe({
      next: () => {
        this.loadCategories();
        this.showDeleteSuccess(this.ENTITY_NAME);
      },
      error: error => {
        this.handleApiError(error, 'Không thể xóa danh mục');
      },
    });
  }
