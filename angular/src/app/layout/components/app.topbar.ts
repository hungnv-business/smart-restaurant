        ...
        <li class="right-sidebar-item static sm:relative">
          ...
          >
            <div class="flex items-center gap-2 justify-between">
              <span class="text-lg font-medium text-surface-950 dark:text-surface-0"
                >Notifications</span
              >
              <button pRipple class="text-surface-700 dark:text-surface-300 text-sm font-medium">
                Mark all as read
              </button>
            </div>
            <div
            ...
            >
              @for (item of notificationsBars(); track item.id; let i = $index) {
                <button
                  [ngClass]="{
                    'bg-primary-100 dark:bg-primary-900': selectedNotificationBar() === item.id,
                    'hover:bg-primary-100/30 dark:hover:bg-primary-900/30':
                ...
                >
                  <span class="capitalize font-medium text-surface-950 dark:text-surface-0">{{
                    item.id
                  }}</span>
                </button>
              }
              ...
              @for (item of selectedNotificationsBarData(); track item.name; let i = $index) {
                <li class="flex gap-3">
                  ...
                  <div class="flex-1 pt-2 space-y-2">
                    ...
                    >
                      <p
                        class="text-sm text-surface-700 dark:text-surface-200 line-clamp-2 text-ellipsis"
                      >
                        {{ item.description }}
                      </p>
                    </div>
