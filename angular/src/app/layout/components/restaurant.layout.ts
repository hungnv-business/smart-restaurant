...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, AppTopbar, AppSidebar, RouterModule, AppBreadcrumb, AppFooter, AppConfigurator, AppRightMenu, AppSearch],
    template: `
            ...
            if (!this.menuOutsideClickListener) {
                this.menuOutsideClickListener = this.renderer.listen('document', 'click', event => {
                    ...
                    const isOutsideClicked = !(
                        event.target.isSameNode(layoutWrapper) || 
                        layoutWrapper?.contains(event.target)
                    );
                    if (isOutsideClicked) {
                        this.hideMenu();
                    }
