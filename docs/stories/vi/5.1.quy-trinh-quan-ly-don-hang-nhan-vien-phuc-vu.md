# User Story 5.1: Quy trình Quản lý Đơn hàng của Nhân viên Phục vụ
...
- [ ] **BE-1**: Thiết kế Mô hình Domain Order với Logic Kinh doanh (AC: 1, 4, 6)
  - [ ] Định nghĩa entity Order với enum OrderStatus (Chờ, Xác nhận, Đang chuẩn bị, Sẵn sàng, Đã phục vụ, Đã thanh toán)
  - [ ] Tạo entity OrderItem và cấu hình mối quan hệ với MenuItem
  - [ ] Thêm validation quy tắc kinh doanh cho chuyển đổi trạng thái đơn hàng và tình trạng bàn
  - [ ] Triển khai domain events cho thay đổi trạng thái đơn hàng và thông báo bếp
  - [ ] Thêm trường audit và đảm bảo kế thừa ABP FullAuditedAggregateRoot phù hợp

- [ ] **BE-2**: Tạo Order Domain Service (AC: 4, 5, 6)
  - [ ] Triển khai OrderDomainService với phương thức ValidateTableAvailabilityAsync
  - [ ] Thêm logic validation chuyển đổi trạng thái (Chờ → Xác nhận → Đang chuẩn bị → Sẵn sàng → Đã phục vụ → Đã thanh toán)
  - [ ] Tạo phương thức CalculateTotalAmountAsync cho tính giá đơn hàng
  - [ ] Triển khai các phương thức NotifyKitchenAsync và PrintKitchenBillAsync
  - [ ] **Thêm phương thức ProcessInventoryDeductionAsync cho tự động trừ kho khi đơn hàng được xác nhận**
  - [ ] Thêm thực thi quy tắc kinh doanh cho quy trình hoàn thành phục vụ

- [ ] **BE-3**: Triển khai Order Application Service (AC: 1, 2, 3, 4, 5, 6, 7)
  - [ ] Tạo OrderAppService sử dụng IApplicationService (không phải ICrudAppService)
  - [ ] Triển khai CreateOrderAsync với validation bàn và kiểm tra món ăn
  - [ ] **Thêm validation tồn kho trong CreateOrderAsync**
  - [ ] Thêm phương thức UpdateOrderStatusAsync với phân quyền phù hợp
  - [ ] **Tích hợp tự động trừ kho khi trạng thái đơn hàng chuyển thành Đã xác nhận**
  - [ ] Tạo các phương thức chuyên biệt GetOrdersForTableAsync và GetKitchenOrdersAsync
  - [ ] Thêm chức năng PrintKitchenBillAsync và ReprintBillAsync
  - [ ] Triển khai CompleteOrderServiceAsync với đặt lại trạng thái bàn
  - [ ] Thêm quản lý transaction và xử lý lỗi toàn diện

- [ ] **BE-4**: Thêm Cấu hình Database và Hiệu suất (AC: 1, 4)
  - [ ] Cấu hình mối quan hệ Order, OrderItem, Table trong SmartRestaurantDbContext
  - [ ] Thêm migration database cho các entity Order và OrderItem
  - [ ] Cấu hình các index hiệu suất: Orders(TableId), Orders(Status), OrderItems(OrderId)
  - [ ] Thêm các ràng buộc database cho quy tắc kinh doanh (chuyển đổi trạng thái hợp lệ)

- [ ] **BE-5**: Tạo SignalR Hubs cho Cập nhật Thời gian thực (AC: 4, 6)
  - [ ] Triển khai KitchenHub cho cập nhật đơn hàng bếp thời gian thực
  - [ ] Tạo TableManagementHub cho phát sóng trạng thái bàn
  - [ ] Thêm OrderStatusHub cho thông báo nhân viên
  - [ ] Cấu hình phân quyền hub với các vai trò nhà hàng phù hợp
  - [ ] Thêm quản lý kết nối và xử lý lỗi

- [ ] **BE-6**: Triển khai Kiểm thử Logic Kinh doanh (AC: Tất cả)
  - [ ] Viết các unit test OrderDomainService cho tất cả quy tắc kinh doanh
  - [ ] Kiểm thử chuyển đổi trạng thái đơn hàng và logic validation
  - [ ] **Kiểm thử các kịch bản tự động trừ kho và validation tồn kho**
  - [ ] Tạo các integration test cho quy trình đơn hàng hoàn chỉnh
  - [ ] Kiểm thử tích hợp bếp và chức năng in ấn
  - [ ] Thêm các test hiệu suất cho cập nhật thời gian thực

- [ ] **BE-7**: Tạo Entity MenuItemIngredient và Hệ thống Recipe (AC: 2, 4)
  - [ ] Tạo entity MenuItemIngredient với mối quan hệ nhiều-nhiều
  - [ ] Thêm các thuộc tính MenuItemIngredient: MenuItemId, IngredientId, RequiredQuantity, IsOptional
  - [ ] Cập nhật entity MenuItem với thuộc tính điều hướng Ingredients
  - [ ] Cấu hình mối quan hệ Entity Framework cho MenuItemIngredient
  - [ ] Tạo migration database cho bảng MenuItemIngredients

- [ ] **BE-8**: Triển khai RecipeManager Domain Service (AC: 2, 4)
  - [ ] Tạo RecipeManager với phương thức CheckIngredientAvailabilityAsync
  - [ ] Triển khai CalculateRequiredIngredientsAsync cho các items đơn hàng
  - [ ] Thêm ProcessAutomaticDeductionAsync với hỗ trợ kho âm
  - [ ] Tạo GetMissingIngredientsAsync cho hiển thị cảnh báo
  - [ ] Thêm logic kinh doanh cho nguyên liệu tùy chọn và bắt buộc

- [ ] **BE-9**: Cập nhật Hệ thống Kho cho Kho âm (AC: 2, 4)
  - [ ] Loại bỏ ràng buộc kho >= 0 từ entity Ingredient
  - [ ] Cập nhật IngredientAppService cho phép thao tác kho âm
  - [ ] Thêm các phương thức tính toán nguyên liệu thiếu
  - [ ] Tạo migration dữ liệu để migrate từ PrimaryIngredientId sang MenuItemIngredients
  - [ ] Cập nhật logic hiển thị kho để hiện giá trị âm

### Nhiệm vụ Flutter Mobile

- [ ] **FLUTTER-1**: Tạo Màn hình Quy trình Đơn hàng Nhiều bước (AC: 1, 2, 3)
  - [ ] Tạo các màn hình quy trình order: flutter_mobile/lib/features/orders/screens/
  - [ ] Triển khai UI stepper cho: Chọn Bàn → Duyệt Menu → Xem lại Đơn hàng → Xác nhận
  - [ ] Tạo logic điều hướng bước với validation sử dụng Flutter navigation
  - [ ] Thêm chỉ báo tiến trình và nhãn tiếng Việt sử dụng Flutter widgets
  - [ ] Triển khai validation hoàn thành bước và thực thi quy tắc kinh doanh

- [ ] **FLUTTER-2**: Xây dựng Màn hình Chọn Bàn Thân thiện Cảm ứng (AC: 1)
  - [ ] Tạo table_selection_screen.dart với các chỉ báo trạng thái mã màu
  - [ ] Triển khai bố cục lưới responsive cho bàn sử dụng Flutter GridView
  - [ ] Thêm cập nhật trạng thái bàn thời gian thực qua kết nối WebSocket/SignalR
  - [ ] Tạo chú giải trạng thái bàn: Trống (xanh lá), Có khách (đỏ), Đã đặt (vàng), Đang dọn (cam)
  - [ ] Thêm chức năng lọc và tìm kiếm bàn sử dụng Flutter search widgets

- [ ] **FLUTTER-3**: Triển khai Màn hình Duyệt Menu và Tìm kiếm (AC: 2, 3)
  - [ ] Tạo menu_browser_screen.dart với tab danh mục và danh sách món
  - [ ] Triển khai tìm kiếm tiếng Việt với tự động hoàn thành sử dụng Flutter search delegate
  - [ ] Thêm lọc danh mục với chỉ báo trạng thái có sẵn
  - [ ] Tạo chọn món với điều khiển điều chỉnh số lượng sử dụng Flutter widgets
  - [ ] Thêm nhập ghi chú món riêng và chung sử dụng Flutter text fields

- [ ] **FLUTTER-4**: Thêm Màn hình Tóm tắt và Xác nhận Đơn hàng (AC: 3, 4, 7)
  - [ ] Tạo order_summary_screen.dart với danh sách chi tiết và giá
  - [ ] Triển khai điều khiển sửa đổi đơn hàng (thay đổi số lượng, xóa món)
  - [ ] Thêm hiển thị thông tin bàn và ghi chú
  - [ ] Tạo dialog xác nhận với xem lại đơn hàng sử dụng Flutter dialogs
  - [ ] Thêm tích hợp nút thanh toán cho Epic 8

- [ ] **FLUTTER-5**: Xây dựng Màn hình Theo dõi Trạng thái và Tích hợp Bếp (AC: 4, 5, 6)
  - [ ] Tạo order_status_screen.dart với badge trạng thái tiếng Việt
  - [ ] Triển khai cập nhật trạng thái thời gian thực qua WebSocket subscription
  - [ ] Thêm chức năng in thủ công với giao diện chọn món
  - [ ] Tạo giao diện hoàn thành dịch vụ với điều khiển xác nhận
  - [ ] Thêm dòng thời gian lịch sử trạng thái sử dụng Flutter timeline widgets

- [ ] **FLUTTER-6**: Triển khai Tính năng Mobile Nâng cao và Xử lý Lỗi (AC: Tất cả)
  - [ ] Thêm phần tử UI có điều kiện dựa trên trạng thái đơn hàng và quy tắc kinh doanh
  - [ ] Triển khai thông báo validation tiếng Việt và xử lý lỗi
  - [ ] Tạo quy trình xác nhận cho thao tác quan trọng sử dụng Flutter alerts
  - [ ] Thêm trạng thái tải và chỉ báo tiến trình sử dụng Flutter progress widgets
  - [ ] Triển khai phát hiện offline và thông báo sử dụng Flutter connectivity
  - [ ] **Xử lý các tình huống edge case: Timeout mạng, mất kết nối WebSocket, trạng thái bàn không hợp lệ, lỗi máy in bếp**
  - [ ] **Thêm quy trình phục hồi lỗi toàn diện: Tự động thử lại đơn hàng thất bại, tiếp tục workflow bị gián đoạn, đồng bộ dữ liệu sau khi kết nối lại**
  - [ ] **Thêm tính năng accessibility: Hỗ trợ đọc màn hình, chế độ tương phản cao, hỗ trợ text lớn, kích thước mục tiêu cảm ứng**

- [ ] **FLUTTER-7**: Thêm Hệ thống Cảnh báo Thiếu Nguyên liệu (AC: 2, 4)
  - [ ] Tạo popup cảnh báo thiếu nguyên liệu với thông báo tiếng Việt
  - [ ] Hiển thị nguyên liệu thiếu cụ thể cho từng món: "Phở Bò: thiếu Thịt bò (cần 200g)"
  - [ ] Thêm dialog xác nhận: "Đặt món và cần mua thêm nguyên liệu"
  - [ ] Triển khai màn hình danh sách nguyên liệu thiếu thời gian thực
  - [ ] Hiển thị giá trị kho âm với chỉ báo rõ ràng: "Thịt bò: -500g"
...
- [ ] **TEST-3**: Kiểm thử Tích hợp Thời gian thực (AC: 4, 6)
...
**Tích hợp Bếp** [Nguồn: docs/architecture/core-workflows.md#Kitchen-Workflow]:
...
_Sẽ được điền bởi development agent trong quá trình triển khai_
...
| 2025-09-04 | 1.5 | Loại bỏ dependency Redis, thay bằng ABP Memory Cache | Bob (Scrum Master) |
