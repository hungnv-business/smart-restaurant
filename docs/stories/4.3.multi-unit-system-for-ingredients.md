...
### Backend Tasks
- [ ] **BE-0**: Create data migration strategy for existing ingredients from Story 4.2
- [ ] **BE-1**: Create IngredientPurchaseUnit entity with int ConversionRatio
- [ ] **BE-2**: Enhance Ingredient entity with PurchaseUnits navigation property
- [ ] **BE-3**: Create database migration for IngredientPurchaseUnits table with performance indexes
- [ ] **BE-4**: Update PurchaseInvoiceItems with multi-unit columns (maintain decimal price compatibility)
- [ ] **BE-5**: Update MenuItem with PrimaryIngredientId, RequiredQuantity columns  
- [ ] **BE-6**: Enhance IngredientAppService with purchase unit management methods
- [ ] **BE-7**: Update CreateUpdateIngredientDto with PurchaseUnits list property (int conversion ratio)
...
### Testing Tasks
- [ ] **TEST-1**: Unit tests for conversion calculations with int types
- [ ] **TEST-2**: Integration tests for ingredient creation with purchase units
...
- [ ] **TEST-5**: Data migration testing with existing ingredients
